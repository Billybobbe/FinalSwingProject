...
public class PhoneProjectile extends Projectile{
    ...
    public void act() throws UnsupportedAudioFileException, LineUnavailableException, IOException {
        ...
        if(timer/60 >= splitTimeInSeconds){
            Projectile p = new Projectile(Resource.PHONE, getX(), getY(), 15, 20, speedX-(0.5*speedX), speedY);
            Projectile p1 = new Projectile(Resource.PHONE_CASE, getX(), getY(), 15, 20, speedX+(0.5*speedX), speedY);
